...
const friendHasValidKeys = objectHasInvalidKeys.bind(
  null,
  requiredFriendKeys.concat(optionalFriendKeys)
);
...
class AuthApi {
  ...
  getOAuthRequestToken() {
    return new Promise((fulfill, reject) => {
      this.__auth.getOAuthRequestToken(function (
        err,
        oAuthToken,
        oAuthTokenSecret,
        results
  ...
  getOAuthAccessToken(oAuthToken, oAuthTokenSecret, oAuthVerifier) {
    return new Promise((fulfill, reject) => {
      this.__auth.getOAuthAccessToken(
        oAuthToken,
        oAuthTokenSecret,
        oAuthVerifier,
...
class SplitwiseApi {
  ...
  parseSentence(sentence, friendId, autosave) {
    ...
    return this.__authPost(
      `https://secure.splitwise.com/api/v3.0/parse_sentence?${paramString}`,
      null
    );
  }
  ...
  getUser(userId) {
    return this.__authGet(
      `https://secure.splitwise.com/api/v3.0/get_user/${userId}`
    );
  }
  ...
  updateUser(userId, user) {
    ...
    return this.__authPut(
      `https://secure.splitwise.com/api/v3.0/update_user/${userId}?${paramString}`,
      null
    ).then((data) => data["user"]);
  }
  ...
  createGroup(name, members, groupType, countryCode) {
    ...
    return this.__authPost(
      `https://secure.splitwise.com/api/v3.0/create_group?${paramString}`,
      null
    ).then((data) => {
  ...
  deleteGroup(groupId) {
    return this.__authPost(
      `https://secure.splitwise.com/api/v3.0/delete_group/${groupId}`
  ...
  addUserToGroup(groupId, user) {
    ...
    return this.__authPost(
      `https://secure.splitwise.com/api/v3.0/add_user_to_group?${paramString}`,
      null
    ).then((data) => {
      if (!data.success) {
  ...
  removeUserFromGroup(groupId, userId) {
    ...
    return this.__authPost(
      `https://secure.splitwise.com/api/v3.0/remove_user_from_group?${paramString}`,
      null
    ).then((data) => {
      if (!data.success) {
        return Promise.reject({ errors: data.errors });
  ...
  getExpenses(expensesConfig) {
    ...
    if (expensesConfig) {
      const invalidKeysError = objectHasInvalidKeys(
        validExpenseKeys,
        expensesConfig
      );
      if (invalidKeysError) {
    ...
    return this.__authGet(requestUrl).then((data) =>
  ...
  getExpense(expenseId) {
    return this.__authGet(
      `https://secure.splitwise.com/api/v3.0/get_expense/${expenseId}`
  ...
  createExpense(expense, userShares) {
    ...
    return this.__authPost(
      `https://secure.splitwise.com/api/v3.0/create_expense?${paramString}`,
      expense
    ).then((data) => {
  ...
  updateExpense(expenseId, expenseUpdate) {
    ...
    const userShareUpdate = ["user_id", "paid_share", "owed_share"];

    const paramString = encodeAsUrlParam(expenseUpdate);

    return this.__authPost(
      `https://secure.splitwise.com/api/v3.0/update_expense/${expenseId}?${paramString}`,
      null
    ).then((data) => {
      if (data.errors && Object.keys(data.errors).length > 0) {
        return Promise.reject(data.errors);
  ...
  getFriend(friendId) {
    return this.__authGet(
      `http://secure.splitwise.com/api/v3.0/get_friend/${friendId}`
  ...
  createFriend(friend) {
    ...
    return this.__authPost(
      `https://secure.splitwise.com/api/v3.0/create_friend?${urlParams}`,
      friend
    ).then((data) => data["friends"]);
  }
  ...
  createFriends(friends) {
    ...
    return this.__authPost(
      `https://secure.splitwise.com/api/v3.0/create_friends?${urlParams}`
    ).then((data) => {
      return data["friends"];
    });
  }
  ...
  deleteFriend(friendId) {
    return this.__authPost(
      `https://secure.splitwise.com/api/v3.0/delete_friend/${friendId}`
    ).then((data) => data["success"]);
  }
